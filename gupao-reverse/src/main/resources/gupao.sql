...
CREATE TABLE `user_address` (
  ...
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `uk_user_unique_code` (`user_unique_code`)
...
CREATE TABLE `member` (
  ...
  `user_unique_code` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '用户全局唯一编码,所有关联用户的标使用这个字段作为外键',
...
-- 8 会员权益资源关联表
DROP TABLE IF EXISTS `member_right_resource_rel`;
CREATE TABLE `member_right_resource_rel` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `right_id` int(11) unsigned NOT NULL COMMENT '权益ID',
  `right_resource_id` int(11) unsigned NOT NULL COMMENT '权益资源ID',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='会员权益资源关联表';

-- 9 会员权益资源表
DROP TABLE IF EXISTS `member_right_resource`;
CREATE TABLE `member_right_resource` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `resource_type` tinyint(2) unsigned NOT NULL DEFAULT '0' COMMENT '资源分类：1书籍 2优惠券 3文化衫',
  ...
  `picture` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源图片',
  `cost` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '现价，有效价格',
  `price` int(11) NOT NULL DEFAULT '0.00' COMMENT '原价，无效价格',
  `description` blob NOT NULL COMMENT '资源图文描述',
  `receive_num` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '领取人数',
  `invalid_state` tinyint(1) NOT NULL COMMENT '过期状态：0未过期 1已过期',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='会员权益资源表';
...
CREATE TABLE `member_right_rel` (
  ...
  `user_unique_code` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '用户全局唯一编码',
  `member_right_id` int(11) unsigned NOT NULL COMMENT '关联[会员权益字典]的权益ID',
  `gain_limit` tinyint(2) NOT NULL COMMENT '领取次数限制',
  `gain_times` tinyint(2) NOT NULL COMMENT '已领取次数',
  `invalid_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '权益过期时间',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE `member_right_gain_history` (
  ...
  `user_unique_code` varchar(64) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '用户全局唯一编码',
  `member_right_id` int(11) unsigned NOT NULL COMMENT '关联[会员权益字典]的权益ID',
  `resource_id` int(11) unsigned NOT NULL COMMENT '对应会员权益资源表ID',
  `category_id` int(11) NOT NULL COMMENT '资源类型ID',
  `category_code` varchar(64) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '资源类型编码',
...
CREATE TABLE `course_homework_reply` (
  ...
  `user_unique_code` varchar(64) NOT NULL COMMENT '用户唯一编码',
  `lecturer_id` int(11) unsigned DEFAULT NULL COMMENT '出题人,关联讲师ID',
  `homework_reply_content` blob COMMENT '答作业内容',
...
CREATE TABLE `course_homework_reply_praise` (
  ...
  `user_unique_code` varchar(64) NOT NULL COMMENT '用户唯一编码',
  `type` tinyint(1) NOT NULL COMMENT '赞或踩,0赞,1踩',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  KEY `course_homework_reply_id` (`course_homework_reply_id`),
...
CREATE TABLE `user_course` (
  ...
  `user_unique_code` varchar(64) NOT NULL COMMENT '用户唯一编码',
  -- 新增字段
  `order_no` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '订单号 同时也是订单编号',
...
CREATE TABLE `order_logistics` (
  ...
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `track_company_code` varchar(30) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '快递公司编码',
  PRIMARY KEY (`id`) USING BTREE,
...
CREATE TABLE `coupon` (
  ...
  `grade_id` int(11) unsigned DEFAULT NULL COMMENT '所属学科id:(JAVA、大数据,对应字典表) 0表示不限制学科',
